...
function App(){
 ...
 const [playing,setPlaying]=useState(false),[local,setLocal]=useState([]),[fullPlayer,setFullPlayer]=useState(false);
 ...
 const audio=useRef(null),fileInput=useRef(null);
 ...
 useEffect(()=>{const a=audio.current;if(!a)return;a.pause();setPlaying(false);setTime(0);setDuration(0);if(current.src){a.src=current.src;a.load()}else a.removeAttribute('src')},[current]);
 useEffect(()=>{
  ...
  try{
   ...
   safe('play',()=>audio.current?.play().catch(()=>{}));safe('pause',()=>audio.current?.pause());safe('previoustrack',prev);safe('nexttrack',next);
   safe('seekbackward',()=>seekBy(-10));safe('seekforward',()=>seekBy(10));safe('seekto',d=>{if(audio.current&&Number.isFinite(d.seekTime))audio.current.currentTime=d.seekTime});
  }catch{}
 },[current,shuffle,repeat,tracks,time]);
 function play(t){setCurrent(t);setFullPlayer(false);if(t.src)setTimeout(()=>audio.current?.play().catch(()=>{}),120)}
 function next(){if(!tracks.length)return;const i=tracks.findIndex(t=>t.id===current.id);play(tracks[shuffle?Math.floor(Math.random()*tracks.length):(i+1)%tracks.length])}
 ...
 function importMusic(e){
  try{
   ...
   const arr=files.map((f,i)=>({id:`local-${Date.now()}-${i}-${Math.random().toString(36).slice(2,7)}`,title:f.name.replace(/\.[^/.]+$/,''),artist:'Local Music',album:'My Library',color:'#312e81',src:URL.createObjectURL(f)}));
   setLocal(p=>[...arr,...p]);setCurrent(arr[0]);setTab('Library');
  }catch(err){console.error('Music import failed',err)}finally{e.target.value=''}
 }
 ...
 else if(tab==='Library')page=<LibraryPage local={local} openImporter={openImporter} common={common}/>;
 ...
 return <div className="app">
  ...
  <audio ref={audio} preload="metadata" onLoadedMetadata={()=>setDuration(audio.current?.duration||0)} onTimeUpdate={()=>setTime(audio.current?.currentTime||0)} onEnded={onEnded} onPlay={()=>{setPlaying(true);if('mediaSession' in navigator)navigator.mediaSession.playbackState='playing'}} onPause={()=>{setPlaying(false);if('mediaSession' in navigator)navigator.mediaSession.playbackState='paused'}} onError={()=>setPlaying(false)}/>
 </div>
}
...
function LibraryPage({local,openImporter,common}){return <section className="content"><p className="eyebrow">YOUR COLLECTION</p><h1>Library</h1><div className="libraryCard"><div><h2>{local.length} local tracks</h2><p>Your imported music is ready for playback.</p></div><button className="primary" onClick={openImporter}><Upload size={17}/> Import music</button></div>{local.length?<div className="trackList">{local.map(t=><Track key={t.id} t={t}{...common}/>)}</div>:<Empty icon={<Library size={40}/>} text="No local music yet" sub="Import your own audio files to listen offline."/>}</section>}
