...
function openMusicDB(){return new Promise((resolve,reject)=>{const req=indexedDB.open(DB_NAME,DB_VERSION);req.onupgradeneeded=()=>{const db=req.result;if(!db.objectStoreNames.contains(STORE))db.createObjectStore(STORE,{keyPath:'id'});};req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error);});}
async function dbGetAll(){const db=await openMusicDB();return new Promise((resolve,reject)=>{const tx=db.transaction(STORE,'readonly');const req=tx.objectStore(STORE).getAll();req.onsuccess=()=>resolve(req.result||[]);req.onerror=()=>reject(req.error);});}
async function dbPut(row){const db=await openMusicDB();return new Promise((resolve,reject)=>{const tx=db.transaction(STORE,'readwrite');tx.objectStore(STORE).put(row);tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error);});}
...
function App(){
 ...
 function play(t){
   if(!t?.src)return;
   const a=audio.current;
   if(current.id===t.id){if(playing)a?.pause();else a?.play().catch(err=>console.error('Playback failed',err));setFullPlayer(false);return;}
   startTrack(t);setFullPlayer(false);
 }
 function next(){if(!tracks.length)return;const i=tracks.findIndex(t=>t.id===current.id);const start=i<0?0:i;const n=shuffle?tracks[Math.floor(Math.random()*tracks.length)]:tracks[(start+1)%tracks.length];if(n?.src)startTrack(n);}
 function prev(){if(time>4){if(audio.current)audio.current.currentTime=0;return;}const i=tracks.findIndex(t=>t.id===current.id);const start=i<0?0:i;const p=tracks[(start-1+tracks.length)%tracks.length];if(p?.src)startTrack(p);}
 ...
 async function importMusic(e){try{const files=Array.from(e.target.files||[]).filter(f=>f.type.startsWith('audio/')||AUDIO_EXT.test(f.name));if(!files.length)return;const arr=[];for(const f of files){const id=`local-${Date.now()}-${Math.random().toString(36).slice(2,9)}`;const src=URL.createObjectURL(f);urls.current.add(src);const item={id,title:f.name.replace(/\.[^/.]+$/,''),artist:'Local Music',album:'My Library',color:'#312e81',src};await dbPut({id,title:item.title,artist:item.artist,album:item.album,color:item.color,blob:f,name:f.name,type:f.type||'audio/*'});arr.push(item);}setLocal(p=>[...arr,...p]);setTab('Library');}catch(err){console.error('Music import failed',err);alert('Music import failed. Please try the file again.');}finally{e.target.value='';}}
